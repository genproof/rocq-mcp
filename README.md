...
| `ROCQ_SENTENCE_TIMEOUT` | `0` | Global default per-sentence wall-clock budget (seconds) for `rocq_compile_lsp` document checking. `0` disables it; when `> 0`, any single sentence that runs longer is aborted coq-lsp-side and reported as a `Timeout!` error (checking then recovers and continues), so one slow/diverging tactic cannot wedge a check. The `rocq_compile_lsp(sentence_timeout=...)` parameter overrides this per call (pass `0` to force-disable). Requires the `genproof/rocq-lsp` fork; relies on the tactic cooperatively polling Coq's interrupt (essentially all real computation does). |
