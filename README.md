...
- **coq-lsp transport**: `"timeout"`, `"crashed"`, `"memory_exhausted"` (coq-lsp RSS exceeded the cap; the response also carries `lsp_restarted: True`), `"hard_timeout"` (an operation exceeded `ROCQ_HARD_TIMEOUT` and coq-lsp was killed + restarted; also carries `lsp_restarted: True`), `"stall_timeout"` (coq-lsp made no `$/coq/fileProgress` for `sentence_timeout + ROCQ_PROGRESS_GRACE` — a diverging sentence; coq-lsp was killed + restarted, `lsp_restarted: True`, and the response carries `diverging_sentence: {line, character, text}` locating the culprit).
...
| `ROCQ_SENTENCE_TIMEOUT` | `0` | Global default per-sentence wall-clock budget (seconds) for coq-lsp document checking, honored by **every** tool that drives a check (`rocq_compile_lsp` and the goals-driven `rocq_get_state` / `rocq_step` / `rocq_step_multi` / `rocq_query`). `0` disables it; when `> 0`, any single sentence that runs longer is aborted coq-lsp-side and reported as a self-identifying `rocq-lsp: sentence timeout` error (checking then recovers and continues), so one slow/diverging tactic cannot wedge a check. `rocq_compile_lsp(sentence_timeout=...)` overrides this per call (pass `0` to force-disable). Requires the `genproof/rocq-lsp` fork; relies on the tactic cooperatively polling Coq's interrupt (essentially all real computation does). |
