...
By default a snapshot is saved only for a **clean** check: a file with errors still *completes* (coq-lsp recovers from ordinary errors and checks to EOF), but caching a broken state would just warm-start the next session straight back into the same errors. Pass `rocq_compile_lsp(..., cache_on_error=True)` to snapshot a completed-but-erroring document anyway — useful to warm-start a large file whose tail you are still fixing. Position-limited checks (`line` set) never snapshot.
...
| `ROCQ_OP_TIMEOUT` | `30` | Default **per-command budget** (seconds) for the speculative-command (pretac) tools — `rocq_step` / `rocq_step_multi` / `rocq_query` (position) / `rocq_assumptions` — when no explicit `timeout` is given: the coq-side `command_timeout` at which a cooperative command is aborted, and the seed for the command-phase kill window (`+ ROCQ_PROGRESS_GRACE`). Document checks (`rocq_compile_lsp`, `rocq_get_state`, whole-file/preamble `rocq_query`) do **not** use it — they block and are bounded by `ROCQ_SENTENCE_TIMEOUT` + the stall watchdog. (Falls back to the legacy `ROCQ_PET_TIMEOUT`.) |
