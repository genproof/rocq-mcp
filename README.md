...
| **`rocq_get_state`** | Show the proof goals at a `(file, line, character)` position (0-indexed). Positions round **backward** by default (`before=true`): you see the goal the sentence at the point operates on, so pointing at `- admit.` shows the goal it discharges; pass `before=false` for the state after. Returns `goals` (empty when no foreground goals remain) and `in_proof`. Use it to inspect a proof mid-way or at an error position from `rocq_compile`. |
