...
- **coq-lsp transport**: `"timeout"`, `"crashed"`, `"memory_exhausted"` (coq-lsp RSS exceeded the cap; the response also carries `lsp_restarted: True`), `"hard_timeout"` (an operation exceeded `ROCQ_HARD_TIMEOUT` and coq-lsp was killed + restarted; also carries `lsp_restarted: True`).
...
| `ROCQ_HARD_TIMEOUT` | `0` | Hard wall-clock backstop (seconds) for **any** single coq-lsp operation. `0` disables it. When `> 0`, an operation running longer is aborted by **killing and restarting** that session's coq-lsp subprocess (which reloads its `.vof` warm-start on the next call); the response carries `reason: "hard_timeout"` and `lsp_restarted: True`. This is the only lever that frees a *non-cooperative* divergence — a tactic that ignores Coq's interrupt (e.g. `do N idtac` or a monolithic `vm_compute`) — which `ROCQ_SENTENCE_TIMEOUT` cannot. Last-resort safety net; there is no longer a client-side wait timeout. |
...
| `op` | `lsp_op.start` / `.end` / `.memory_exhausted` / `.hard_timeout` / `.cancelled` | Each pooled coq-lsp operation: tool, session key, pid, duration, success, peak RSS. |
